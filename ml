...
さて、通常、何らかの問題を解決しようとする場合――特に何らかのパターンを見つける必要がある場合――、人があれやこれやと考えて答えを出すことが一般的でしょう。「この問題はどうやらこういう規則性があるのではないか」、「いやいや、別の場所に原因があるかもしれない」といったように、人の経験や直感を手がかりに試行錯誤を重ねて仕事を進めます。一方、機械学習による手法では、人の介入を極力避け、集められたデータから答え（パターン）を見つけようと試みます。さらに、ニューラルネットワークやディープラーニングは、従来の機械学習で使われた手法以上に、人の介入を遠ざけることができるという重要な性質を持ちます
...
ディープラーニングは、「end-to-end machine learning」と呼ばれることがあります。ここで言うend-to-end とは、「端から端まで」という意味であり、これは生データ（入力）から目的の結果（出力）を得ることを意味します。



***機械学習で解ける範囲と解けない範囲を線引きする***

人間は、例えば徹夜で連続して単純作業をしていると、その判断基準がぶれてくることが往々にしてあります。それに対し、機械学習の予測基準は学習フェーズからブレることはないので、大量のデータに対しても人より安定した予測を行えるのです。

そもそも解こうと思っている課題が人間にも解けな
い問題の場合は機械学習で解くのも難しいでしょう。機械学習、特に「教師あり学習」では、何が「正解」
なのかを人間が機械に教えなければなりません。つまり、人が「正解」を決められない問題は機械にも
解くことができないのです
...
* 「正解の判断が難しい。」
* 機械学習に100％正解を出力するアルゴリズムは存在しない
機械学習、特にDeepLearningは、モデルを元に大量のデータを使ってパラメータを自動調整する。よって
